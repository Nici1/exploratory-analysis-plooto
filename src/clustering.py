...
class Clustering(AnomalyDetectionAbstract):
    ...
    treshold: float  # the euclidean distance that is required to determine anomalies (the larger the distance the less points are anomalous)
    ...
    retrain_file: str  # Path and file name of the file in which retrain data will be stored
    trained: bool
    ...
    def train_model(
        ...
        if len(features) > 0:
            # train model
            self.model = DBSCAN(eps=self.eps, min_samples=self.min_samples).fit(
                features
            )
