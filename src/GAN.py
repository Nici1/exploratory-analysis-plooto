...
class GAN(AnomalyDetectionAbstract):
    ...
    def message_insert(self, message_value: Dict[Any, Any]) -> Any:
        ...
        if not self.check_ftr_vector(message_value=message_value):
            ...
            #                                filtering    timestamp=message_value["timestamp"])
        ...
        if feature_vector == False:
            ...
            # Model prediction
            prediction = self.GAN.predict(feature_vector.reshape(1, self.N_shifts + 1))[
                0
            ]
    ...
    def mse(pre_GAN, post_GAN):
        ...
        mse = np.sum((np.add(np.array(pre_GAN), -np.array(post_GAN)) ** 2)) / float(
            len(pre_GAN)
        )
        return mse
    ...
    def train_model(
        ...
        if len(features) > 0:
            ...
            print("Shape of array: ", features.shape)
            predictions = self.GAN.predict(
